...
class graph2graph(object):
    ...
    def build_model(self):
        ...
        self.real_A = self.real_data[:, :, :, :self.input_c_dim]  # eg vıew1
        self.real_B = self.real_data[:, :, :, self.input_c_dim:self.input_c_dim + self.output_c_dim] #eg vıew4

        self.fake_B = self.generator(self.real_A,reuse=False,name="generatorA2B") # how to get fakeB   A2B
        ########################aaa
        self.fake_A_ = self.generator(self.fake_B, reuse=True,name="generatorB2A") #from cgan, how to add ıt loss ??  in the paper fake_A(noise) is attained by a dropout in the gen arch    
        self.fake_A = self.generator(self.real_B,  reuse=True,name="generatorB2A") #generatorB2A conditioned
        self.fake_B_= self.generator(self.fake_A,  reuse=True,name="generatorA2B") #from cgan,how to add ıt loss??
        '''
        #self.real_AB = tf.concat([self.real_A, self.real_B], 3)  #v1 and v4   A2B
        #self.fake_AB = tf.concat([self.real_A, self.fake_B], 3)#  v1 and v'  A2B #(1,35, 35)
        #print('concatenated fake AB dimension is', np.shape(self.fake_AB))##  #(1,35, 35,2
        ################aa
        #self.real_BA = tf.concat([self.real_A, self.real_B], 3)  #v1 and v4
        #self.fake_BA = tf.concat([self.real_B, self.fake_A],3)# Critical for cyclic, for cyclic ınput of genA is fake B hence self.fake_A_
        print('concatenated fake BA dimension is', np.shape(self.fake_BA))##  #(1,35, 35,2) 
        '''
         ################
        #self.D= self.discriminator(self.real_AB, reuse=False) 
        #print('self.D is of shape when lonely', np.shape(self.D)) # (2,)
        #print('self.D is', self.D) # 
       # self.D_logits = self.discriminator(self.real_AB, reuse=True) 
        #print('self.D_logits is of shape when lonely', np.shape(self.D_logits))#(2,)
        #print('self.D_logits is', self.D_logits) #
        
        self.D, self.D_logits = self.discriminator(self.real_B, reuse=False,name="discriminatorB")   # dısc fırst ınput
        #thıs ıs self.DB_fake ın CGAN
        self.D_, self.D_logits_ = self.discriminator(self.fake_B, reuse=True,name="discriminatorB")  #dısc  ınput
        ############a
        self.D2, self.D2_logits = self.discriminator(self.real_A, reuse=True,name="discriminatorA")   # dısc fırst ınput
        # This ıs self.DA_fake ın Cgan
        self.D2_, self.D2_logits_ = self.discriminator(self.fake_A, reuse=True,name="discriminatorA")  #dısc  ınput
        ####   
        print('self.D2 is of shape when together', np.shape(self.D2_))#  (1, 1)
        print('self.D2_logıts is of shape when together', np.shape(self.D2_logits))# (1,1)
        ...
        #  when fed wıth fakeB. g_loss_a2b
        #    db fake                                db fake
        ...
        self.g1_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.D_logits_, labels=tf.ones_like(self.D_))) \
                ...
                #+self.L1_lambda*tf.reduce_mean(tf.abs(self.real_B-self.fake_B_))     #  cc loss
        ...
        #  when fed wıth fakeA. g_loss_b2a 
        ...
        print("this is degree realA", degre_tf(self.real_A))
        print("this is degree fakeA", degre_tf(self.fake_A))
        print("this is degree fakeB", degre_tf(self.fake_B))
             #+ self.L1_lambda * tf.reduce_mean(tf.abs(self.real_A - self.fake_A_))
             #+ self.L1_lambda*tf.reduce_mean(tf.abs(self.real_B-self.fake_B_))
        #cyc_loss=tf.reduce_mean(tf.abs(self.real_A-self.fake_A_)) + tf.reduce_mean(tf.abs(self.real_B-self.fake_B_)) 
       
        #self.g_loss=self.g_loss+self.g2_loss # /2 maynot be correct
        tf.enable_eager_execution()
        self.g_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.D_logits_, labels=tf.ones_like(self.D_))) \
             ...
             #+self.L1_lambda * tf.reduce_mean(tf.abs(degre_tf(self.real_A)-degre_tf(self.fake_B)))+self.L1_lambda * tf.reduce_mean(tf.abs(degre_tf(self.real_B)-degre_tf(self.fake_A)))
    ...
    def train(self, args,data):
        ...
        #a = tf.summary.merge([self.d_sum+self.d2_sum, self.d_loss_real_sum+self.d2_loss_real_sum, self.d_loss_sum])#no self.d2_loss_sum]
...
#         data = load_data(args.train_dir,'train',self.graph_size[0],self.dataset)
                   ...
                   for i in range(self.d_train_num):
                    # Update G network
                        _, summary_str = self.sess.run([d_optim, self.d_sum],
                                               feed_dict={ self.real_data: batch_graphs })  # no d2 sum
                ...
                for i in range(self.g_train_num):
                    # Update G network
                     _, summary_str = self.sess.run([g_optim, self.g_sum],
                                               feed_dict={ self.real_data: batch_graphs })  # no g2 sum
        ...
        with open("./losses/errG_totalv32v1nclh.txt", "w") as f:
            for s in errG_total:
                f.write(str(s) +"\n")
        with open("./losses/errD_real_totalv32v1nclh.txt", "w") as f:
            for s in errD_real_total:
                f.write(str(s) +"\n")
        with open("./losses/errD_fake_totalv32v1nclh.txt", "w") as f:
            for s in errD_fake_total:
                f.write(str(s) +"\n")
    
        with open("./losses/dloss_totalv32v1nclh.txt", "w") as f:
            for s in dloss_total:
                f.write(str(s) +"\n")
        with open("./losses/d1_lossv32v1nclh.txt", "w") as f:
            for s in d1_loss:
                f.write(str(s) +"\n")
        with open("./losses/d2_lossv32v1nclh.txt", "w") as f:
            for s in d2_loss:
                f.write(str(s) +"\n")
        ...
        plt.plot(errD_fake_total)
        plt.show()
        plt.title('errorD_fake loss')
        plt.plot(errD_real_total)
        plt.show()
        plt.title('errorD_real')
        '''
    ...
    def test(self, args ,sample_graphs_all): #  need for change when testing
        score=[]
        gen_data=[]
...
#         sample_graphs_all =load_data_test(self.graph_size[0],self.dataset)
        ...
        return gen_data
    def test2(self, args ,sample_graphs_all): #  need for change when testing
        score=[]
        gen_data=[]
