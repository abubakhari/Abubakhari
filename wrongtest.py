...
parser.add_argument('--train_size', dest='train_size', type=int, default='154', help='# graphs used to train')#154
...
def main():
    ...
    for train_index, test_index in loo.split(v1):
        ...
        with tf.Session() as sess:
            ...
            datatest=np.zeros((mal2.shape[0],mal2.shape[1],mal2.shape[2],2))  #  here we dont use any v4 data
            for i in range(reg2.shape[0]):
                datatest[i,:,:,0]=mal2[i]
                datatest[i,:,:,1]=mal2[i]#  forming a paır of test samples all from v1.
            ...
            ground = np.reshape(reg2, (-1,1225)) #  fill the previously defined predicted array with current test data
